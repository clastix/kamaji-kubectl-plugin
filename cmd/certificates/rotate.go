...
func NewRotateCertificates(writer io.Writer, flags *genericclioptions.ConfigFlags, k8sClient client.Client) *cobra.Command {
	...
	cmd := cobra.Command{
		...
		Long: "Rotate one ore more Tenant Control Plane certificates.\n" +
			...
			strings.Join(commands.RotateCertificatesMap.Keys(), ", ") + ".\n" +
		...
		PreRunE: func(*cobra.Command, []string) error {
			...
			case len(certificates) == 0:
				return errors.New("at least one certificate must be specified")
			default:
				for _, arg := range certificates {
					if _, ok := commands.RotateCertificatesMap[arg]; !ok {
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			certsToDelete := make(commands.RotateCertOptions)
			for k, v := range commands.RotateCertificatesMap {
				if all || certSet.Has(k) {
					certsToDelete[k] = v
				}
			}

			if err := (&commands.Helper{Client: k8sClient}).RotateCertificate(cmd.Context(), *flags.Namespace, args[0], certsToDelete); err != nil {
