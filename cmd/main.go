...
import (
	...
	"github.com/clastix/kamaji-kubectl-plugin/cmd/certificates"
	"github.com/clastix/kamaji-kubectl-plugin/cmd/kubeconfig"
	"github.com/clastix/kamaji-kubectl-plugin/cmd/token"
)
...
func main() {
	...
	*configFlags.Namespace, _, _ = clientConfig.Namespace()
	...
	if restErr != nil {
		_, _ = writer.WriteString(fmt.Sprintf("cannot retrieve REST configuration: %s", restErr.Error())) //nolint:perfsprint
		os.Exit(1)
	}
